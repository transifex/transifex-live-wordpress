			...
			<tr class="custom-urls-settings<?php echo $hide_custom_urls_css ?>">
				...
				<td>
					...
					<div class="adds-rewrites<?php echo $hide_add_rewrites ?>">
						...
						<input id="transifex_live_options_all" name="transifex_live_options[all]" value="1" type="checkbox">All
						<br/><br/>
							<?php Transifex_Live_Integration_Settings_Util::render_url_options( $rewrite_options_array ); ?>
					</div>
				</td></tr>
			<tr class="custom-urls-settings<?php echo $hide_custom_urls_css ?>">
				<th scope="row" class="titledesc adds-rewrites">Language/region Codes</th>
				<th scope="row" class="titledesc adds-rewrites-subdomain hide-if-js">Subdomain Names</th>
				<td>
					<p class="adds-rewrites">You can customize the language or region code used in your language/region-specific URLs. The code you choose will always appear immediately after your domain. So if you use <code>fr</code> for your French pages, the URL for your Product page will look something like <code><?php echo $site_url_subdirectory_example ?>/product/</code>.</p>
					<p class="adds-rewrites-subdomain hide-if-js">If you've already set up language subdomains on your site (this has to be done outside of the plugin), enter the language subdomain names below. So if <code><?php echo $site_url_subdomain_example ?></code> is the subdomain for your French site, put in <code>fr</code>. When the hreflang tags are automatically added to your site’s header, they will point to each of your language subdomains.</p>
					<br/>
					...
					<p class="adds-rewrites-subdomain">Having trouble getting language/region-specific URLs working? Visit the <a href="/wp-admin/options-permalink.php">Permalinks Settings</a> then return here to clear the WordPress cache, or <a href="https://www.transifex.com/contact/">contact us</a>.</p>
				</td>
			</tr>
			</tbody>
        </table>
