...
// TODO Potential security issue with direct access to $_POST filter it - @mjjacko 4/25/15
    public function options_page() {
        if (isset($_POST['transifex_live_nonce']) && wp_verify_nonce($_POST['transifex_live_nonce'], 'transifex_live_settings')) {
            if (isset($_POST['transifex_live_settings'])){
                update_option('transifex_live_settings', $_POST['transifex_live_settings']);
                echo ('<p>' . _e('Your changes to the settings have been saved!', $this->text_domain).'</p>');
            }
            if (isset($_POST['transifex_live_colors'])){
                update_option('transifex_live_colors', $_POST['transifex_live_colors']);
                echo ('<p>'. _e('Your changes to the colors have been saved!',$this->text_domain).'</p>');
            }
        }
        ...
        <?php } ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <input name="transifex_live_settings[custom_picker_id]" type="text" id="transifex_live_settings_custom_picker_id" value="<?php if ($settings['picker'] == 'custom id') {
